...
const commitMessages = ["New Release : Othman kharbouch 1.0.0"];
...
function generateRandomJSCode() {
  ...
  const snippets = [
    ...
    `document.querySelector("${random(elements)}").addEventListener("${random(
      events
    )}", function() {
...
function showProgress(commitCount, totalCommits, percentage) {
  ...
  if (calculatedPercentage > percentage) {
    ...
    const progressBar = `[${"█".repeat(filledBarLength)}${"-".repeat(
      emptyBarLength
    )}]`;
    ...
    if (percentage >= 100) {
      ...
      process.stdout.write(
        chalk.blueBright(`▻ Progress: ${progressBar} ${percentage}%`)
      );
    }
  }
...
async function simulateDayCommits(date, numCommits, commitCount, totalCommits) {
  for (let i = 0; i < numCommits; i++) {
    const commitTime = dayjs(date)
      .add(lodash.random(0, 23), "hour")
      .add(lodash.random(0, 59), "minute")
      .format();
...
async function generateCommitsOverTime(
  startDate,
  endDate,
  minCommits,
  maxCommits,
  ...
  if (totalDays > 730) {
    let validation = await askQuestion(
      chalk.bgRed(
        "⚠ Warning: Generating more than 2 year of commits might take a very long time. Are you sure you want to continue ? [yes/no] :"
      ) || "yes"
    );
    if (
  ...
  } else if (totalDays > 31 * 6) {
    console.warn();
    let validation = await askQuestion(
      chalk.yellow(
        "⚠ Warning: Generating more than 6 will take a while, are you sure you want to continue ? [yes/no]:"
      ) || "yes"
    );
    if (
      validation.toLowerCase() !== "yes" &&
  ...
  try {
    while (currentDate.isBefore(end) || currentDate.isSame(end)) {
      if (choice == "realistic") {
        ...
        commitCount = await simulateDayCommits(
          currentDate,
          numCommits,
          commitCount,
          totalCommits
...
function showEstimatedTime(estimatedTime) {
  ...
  if (seconds > 0) {
    timeMessage += `${minutes > 0 ? " and " : ""}${seconds} second${
      seconds > 1 ? "s" : ""
    }`;
  }
...
async function main() {
  ...
  try {
    ...
    const endDateInput =
      (await askQuestion(
        "\n▻ Enter the end date (YYYY-MM-DD), ( default is today ): "
      )) || dayjs().format("YYYY-MM-DD");
    const endDate = validateDate(endDateInput);

    const realisticCommitChoice =
      (await askQuestion(
        "\n▻ Would you like to leave some empty days for more realistic commit graph ?\n( [yes]: 0-10 commits per day range )( [no]: Enter custom range ) [yes/no]: "
      )) || "yes";
    let minCommits, maxCommits;

    if (
      realisticCommitChoice.toLowerCase() === "yes" ||
      realisticCommitChoice.toLowerCase() === "y"
    ) {
      ...
      commitChoice = "custom";
      minCommits = parseInt(
        await askQuestion("\n▻ Enter the minimum number of commits per day: "),
        10
      );
      maxCommits = parseInt(
    ...
    const autoPushChoice =
      (await askQuestion(
        "\n▻ Do you want to automatically push the generated commits to your repository? (yes/no): "
      )) || "yes";
